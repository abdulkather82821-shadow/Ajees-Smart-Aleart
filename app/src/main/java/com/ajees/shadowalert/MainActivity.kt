...
class MainActivity : ComponentActivity() {
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContent { ShadowAlertApp() }
    }
}

@Composable
fun ShadowAlertApp(vm: ShadowAlertViewModel = viewModel()) {
    MaterialTheme {
        var tab by remember { mutableIntStateOf(0) }
        Scaffold(bottomBar = {
            NavigationBar { listOf("Home", "Apps", "Analytics", "Settings").forEachIndexed { i, label ->
                NavigationBarItem(selected = tab == i, onClick = { tab = i }, icon = {}, label = { Text(label) })
            }}
        }) { pad ->
            when (tab) {
                0 -> HomeScreen(vm, Modifier.padding(pad))
                1 -> AppsScreen(vm, Modifier.padding(pad))
                2 -> AnalyticsScreen(vm, Modifier.padding(pad))
                else -> SettingsScreen(vm, Modifier.padding(pad))
            }
        }
    }
}

@Composable fun HomeScreen(vm: ShadowAlertViewModel, modifier: Modifier = Modifier) {
    Column(modifier.fillMaxSize().padding(20.dp)) {
        Text("AS SHADOW ALERT", style = MaterialTheme.typography.headlineMedium)
        Text("Use with intention. Live with balance.")
        Spacer(Modifier.height(20.dp))
        Card(Modifier.fillMaxWidth()) { Column(Modifier.padding(18.dp)) { Text("Screen Time Today"); Text("3h 24m", style = MaterialTheme.typography.displaySmall); Text("Keep your time intentional.") } }
        Spacer(Modifier.height(18.dp)); Text("Monitored Applications", style = MaterialTheme.typography.titleLarge)
        vm.apps.forEach { app -> AppCard(app) }
    }
}
...
class ShadowAlertViewModel(private val context: Context) : androidx.lifecycle.ViewModel() {
    ...
    fun openOverlay() { context.startActivity(Intent(Settings.ACTION_MANAGE_OVERLAY_PERMISSION).setData(android.net.Uri.parse("package:${context.packageName}")).addFlags(Intent.FLAG_ACTIVITY_NEW_TASK)) }
}
